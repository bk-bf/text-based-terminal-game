...
def test_conditions_system():
    """Test the conditions system"""
    print("=== Testing Conditions System ===")
    
    # Mock player state for testing
    class MockSurvival:
        def __init__(self):
            self.hunger = 150  # Hungry
            self.thirst = 400  # Normal
            self.fatigue = 100  # Very tired
    ...
    class MockPlayerState:
        def __init__(self):
            self.survival = MockSurvival()
    
    player_state = MockPlayerState()
    manager = get_conditions_manager()
    
    # Evaluate conditions
    active_conditions = manager.evaluate_conditions(player_state)
    print(f"Active conditions: {active_conditions}")
    
    # Calculate total effects
    total_effects = manager.calculate_total_effects(active_conditions)
    print(f"Total effects: {total_effects}")
    
    # Test condition formatting
    for condition in active_conditions:
        formatted = manager.format_condition_for_display(condition)
        print(f"Formatted condition: {formatted}")


if __name__ == "__main__":
    test_conditions_system()